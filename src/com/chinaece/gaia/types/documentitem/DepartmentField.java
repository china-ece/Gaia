...
public class DepartmentField extends ItemType {
	private AutoCompleteTextView autocompletetextview = null;
	private HashMap<String, JSONObject> departments = new HashMap<String, JSONObject>();
	...
	public View getMappingInstance(Context context) throws JSONException {
		...
		if (display == 1 || display == 3 || display == 4) {
			...
			departments.put(displayValue, original);
		...
		autocompletetextview.setValidator(new Validator() {
			...
			public boolean isValid(CharSequence text) {
				return departments.keySet().contains(text.toString());
			}
