...
public class FlowPathActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ArrayList<BranchType> branches = (ArrayList<BranchType>) bundle
				.get("branches");
				...
				if (mode == 0) {
					if (checkName.length() > 0) {
						...
						try {
							...
							sub.put("isToPerson",
									checkboxsdataValue != null ? "true"
											: "false");
							sub.put("userids",
									checkboxsdataValue != null ? checkboxsdataValue
											.toString() : "");
							submitTo.put(sub);
							...
							formatUrl = new URL(DataStorage.properties
									.getProperty("url"));
							SubmitTask submittask = new SubmitTask();
							submittask.execute(formatUrl.toString(),
									token.toString(), submit.toString());
						} catch (Exception e) {
							e.printStackTrace();
						}
					} else {
						Toast.makeText(FlowPathActivity.this, "请选择提交路径",
	...
	class ChoiceDialog extends Dialog implements
	...
	class SubmitTask extends AsyncTask<String, Integer, Boolean> {
		...
		protected Boolean doInBackground(String... params) {
			...
			Boolean flag = OaApi.submitDocumnet(params[1], params[2]);
			return flag;
		}
		...
		protected void onPostExecute(Boolean flag) {
			if (flag) {
				Toast.makeText(FlowPathActivity.this, "提交成功",
						Toast.LENGTH_SHORT).show();
				Intent intent = new Intent(FlowPathActivity.this,
						MainActivity.class);
				startActivity(intent);
				FlowPathActivity.this.finish();
			} else {
				Toast.makeText(FlowPathActivity.this, "提交失败", Toast.LENGTH_LONG)
						.show();
			}
			dialog.dismiss();
		}
